...
// ── Dia civil em Lisboa ────────────────────────────────────────────
// Os epochs .NET "/Date(ms)/" da FPG codificam meia-noite em hora de Lisboa;
// no horário de verão (UTC+1) isso é 23:00 UTC do dia ANTERIOR. Converter com
// toISOString()/getters UTC dá o dia -1 para qualquer data entre fim de Março
// e fim de Outubro — e getters locais só acertam se a máquina estiver no fuso
// de Lisboa (GitHub Actions corre em UTC). Formatar sempre em Europe/Lisbon.
const lisbonDayFmt = new Intl.DateTimeFormat("en-CA", {
  timeZone: "Europe/Lisbon", year: "numeric", month: "2-digit", day: "2-digit",
});

/** ms (number) ou Date → "YYYY-MM-DD" no dia civil de Lisboa, ou null. */
function lisbonCivilDay(input) {
  const d = input instanceof Date ? input : new Date(Number(input));
  if (isNaN(d.getTime())) return null;
  return lisbonDayFmt.format(d); // locale en-CA → YYYY-MM-DD
}

/** "/Date(ms)/" → "YYYY-MM-DD" (dia civil de Lisboa), ou null. */
function parseDateOnlyStr(s) {
  if (!s || typeof s !== "string") return null;
  const m = s.match(/\/Date\((-?\d+)\)\//);
  if (!m) return null;
  return lisbonCivilDay(parseInt(m[1], 10));
}

function fmtDate(d) {
  const iso = d ? lisbonCivilDay(d) : null;
  if (!iso) return "";
...
module.exports = {
  norm, esc, parseDotNetDate, lisbonCivilDay, parseDateOnlyStr, fmtDate, toNum, pickHcpFromRow, isMeaningful,
