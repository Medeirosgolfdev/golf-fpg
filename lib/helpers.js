...
// "YYYY-MM-DD[ ...]" → Date a meia-noite UTC (independente do fuso da máquina)
function parseDateOnlyStr(s) {
  const m = typeof s === "string" && s.match(/^(\d{4})-(\d{2})-(\d{2})/);
  if (!m) return null;
  return new Date(Date.UTC(+m[1], +m[2] - 1, +m[3]));
}

// Os epochs /Date(...)/ da FPG codificam meia-noite em hora de Lisboa; no horário
// de verão (UTC+1) isso é 23:00 UTC do dia anterior — formatar em UTC dava -1 dia.
// Converte para o dia civil de Lisboa, devolvido como meia-noite UTC.
const _lisbonDayFmt = new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Lisbon" });
function lisbonCivilDay(d) {
  if (!d) return null;
  return parseDateOnlyStr(_lisbonDayFmt.format(d));
}

/** ms (number) ou Date → "YYYY-MM-DD" no dia civil de Lisboa, ou null.
 *  Variante string de lisbonCivilDay, para os scrapers em scripts/ que
 *  escrevem datas ISO directamente nos JSON. Aceita epochs negativos
 *  (birthdates pré-1970). */
function lisbonCivilDayStr(input) {
  const d = input instanceof Date ? input : new Date(Number(input));
  if (isNaN(d.getTime())) return null;
  return _lisbonDayFmt.format(d); // locale en-CA → YYYY-MM-DD
}
...
function pickScorecardRec(json) {
  // Aceita 3 formatos:
  //   1. { d: { Records: [rec] } } — wrap completo do .NET (formato browser antigo)
  //   2. { Records: [rec] }        — wrap meio (também antigo)
  //   3. rec já desempacotado      — formato novo (scorecards.json em fpg-scrape-node.js,
  //                                  que guarda directamente o `d.Records[0]`)
  if (!json) return null;
  const p = json?.d ?? json;
  if (p?.Records?.[0]) return p.Records[0];
  // Heurística: se tem campos típicos de scorecard (par_1, gross_1...) é o record.
  if (p?.par_1 !== undefined || p?.gross_1 !== undefined || p?.score_id !== undefined) return p;
  return null;
}

const fsForLoaders = require("fs");
const pathForLoaders = require("path");

/**
 * Carrega rondas WHS de um directório de jogador, aceitando 2 formatos:
 *   - whs.json  (novo, fpg-scrape-node.js): array directa de records
 *   - whs-list.json (antigo, browser scrape): { d: { Records: [...] } }
 * Devolve array de records ou null se nenhum existe.
 */
function loadWhsRows(baseDir) {
  const newPath = pathForLoaders.join(baseDir, "whs.json");
  if (fsForLoaders.existsSync(newPath)) {
    try {
      const j = JSON.parse(fsForLoaders.readFileSync(newPath, "utf-8"));
      return Array.isArray(j) ? j : ((j?.d ?? j)?.Records || []);
    } catch {}
  }
  const oldPath = pathForLoaders.join(baseDir, "whs-list.json");
  if (fsForLoaders.existsSync(oldPath)) {
    try {
      const j = JSON.parse(fsForLoaders.readFileSync(oldPath, "utf-8"));
      return (j?.d ?? j)?.Records || j?.Records || [];
    } catch {}
  }
  return null;
}

/** Devolve o caminho efectivo do WHS file (whs.json novo prevalece). */
function whsFilePath(baseDir) {
  const n = pathForLoaders.join(baseDir, "whs.json");
  if (fsForLoaders.existsSync(n)) return n;
  const o = pathForLoaders.join(baseDir, "whs-list.json");
  if (fsForLoaders.existsSync(o)) return o;
  return null;
}

/**
 * Carrega scorecards de um jogador como Map<score_id, rec>. Aceita 2 formatos:
 *   - scorecards.json (novo): { score_id: rec, ... } single file
 *   - scorecards/{score_id}.json (antigo): um ficheiro por scorecard
 * Faz merge dos dois — se ambos existem, novo prevalece (mais recente).
 */
function loadScorecardsByScoreId(baseDir) {
  const map = new Map();
  // 1) Formato novo
  const newPath = pathForLoaders.join(baseDir, "scorecards.json");
  if (fsForLoaders.existsSync(newPath)) {
    try {
      const obj = JSON.parse(fsForLoaders.readFileSync(newPath, "utf-8"));
      for (const [sid, raw] of Object.entries(obj)) {
        const rec = pickScorecardRec(raw);
        if (rec) map.set(String(sid), rec);
      }
    } catch {}
  }
  // 2) Formato antigo (preencher só o que faltar)
  const oldDir = pathForLoaders.join(baseDir, "scorecards");
  if (fsForLoaders.existsSync(oldDir)) {
    try {
      for (const f of fsForLoaders.readdirSync(oldDir).filter(f => f.endsWith(".json"))) {
        const sid = pathForLoaders.basename(f, ".json");
        if (map.has(sid)) continue;
        try {
          const j = JSON.parse(fsForLoaders.readFileSync(pathForLoaders.join(oldDir, f), "utf-8"));
          const rec = pickScorecardRec(j);
          if (rec) map.set(sid, rec);
        } catch {}
      }
    } catch {}
  }
...
function getPlayedAt(rec, whsRow) {
  return (
    // Preferir as strings de data da FPG (dia civil exacto, sem fuso)
    parseDateOnlyStr(whsRow?.hcp_dateStr) ||
    parseDateOnlyStr(whsRow?.score_dateStr) ||
    // Fallback aos epochs .NET, normalizados para o dia civil de Lisboa
    lisbonCivilDay(parseDotNetDate(rec?.played_at)) ||
    lisbonCivilDay(parseDotNetDate(whsRow?.date)) ||
    lisbonCivilDay(parseDotNetDate(whsRow?.played_at)) ||
    lisbonCivilDay(parseDotNetDate(whsRow?.hcp_date)) ||
    null
  );
}
...
function normalizeWhsRows(rows) {
  for (const r of rows) {
    ...
    // score_id / id — CUIDADO: registos admin (Atribuição/Alteração HCP) têm
    // score_id=null propositadamente. Não sobrescrever com o id interno,
    // senão entram no whsByScoreId e geram rondas fantasma.
    if (r.score_id == null && r.id != null) {
      // Só copiar id → score_id se NÃO for registo administrativo
      const tn = (r.tourn_name || r.tournament_description || '').toLowerCase();
      if (!/atribui|altera/.test(tn) && r.tournament_code !== '000000000') {
        r.score_id = r.id;
      }
    }
    if (r.id == null && r.score_id != null) r.id = r.score_id;
...
module.exports = {
  norm, esc, parseDotNetDate, parseDateOnlyStr, lisbonCivilDay, lisbonCivilDayStr, fmtDate, toNum, pickHcpFromRow, isMeaningful,
  ...
  loadWhsRows, loadScorecardsByScoreId, whsFilePath
};
