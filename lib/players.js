// lib/players.js â€” Discover and load player data
const fs = require("fs");
...
function discoverPlayers(outputRoot, currentFed) {
  ...
  // Listar dirs com WHS scraped (whs.json novo OU whs-list.json antigo)
  // Filtrar APENAS os que existem em players.json — evita processar legados
  // de jogadores removidos. Para reincluir alguém: meter de volta em players.json.
  const playersDbKeys = new Set(Object.keys(playersDb));
  const dirs = fs.readdirSync(outputRoot).filter(d => {
    const full = path.join(outputRoot, d);
    if (!fs.statSync(full).isDirectory()) return false;
    if (!playersDbKeys.has(d)) return false;  // só os que estão em players.json
    return fs.existsSync(path.join(full, "whs.json"))
        || fs.existsSync(path.join(full, "whs-list.json"));
  });
  for (const fed of dirs) {
    ...
    if (!name) {
      try {
        // Aceita whs.json (novo) ou whs-list.json (antigo)
        const { loadWhsRows } = require("./helpers");
        const rows = loadWhsRows(path.join(outputRoot, fed)) || [];
        const sources = [rows[0]].filter(Boolean);
