...
function extractPlayerStats(fed, outputRoot) {
  ...
  // Aceita ambos os formatos de scrape (whs.json novo e/ou whs-list.json antigo)
  ...
  // Aceita scorecards.json (novo) e/ou scorecards/ (antigo)
  const cardByScoreId = loadScorecardsByScoreId(baseDir);
  // Aplicar melhorias a cada scorecard
  for (const [sid, rec] of cardByScoreId.entries()) {
...
/**
 * Calcula um fingerprint rápido baseado nos mtimes dos whs-list.json de todos os jogadores.
 * Se nenhum WHS mudou desde a última geração, o cache é válido.
 */
function computeCacheFingerprint(allPlayers, outputRoot) {
  let hash = allPlayers.length.toString();
  for (const p of allPlayers) {
    // Aceita whs.json (novo) ou whs-list.json (antigo) — usa o que existir
    const baseDir = path.join(outputRoot, p.fed);
    const whsPath = whsFilePath(baseDir);
    try {
      const st = whsPath ? fs.statSync(whsPath) : null;
      hash += "|" + p.fed + ":" + (st ? st.mtimeMs : 0);
    } catch {
      hash += "|" + p.fed + ":0";
    }
  }
...
function extractAllPlayerStats(allPlayers, outputRoot) {
  const cachePath = path.join(outputRoot, "cross-stats-cache.json");

  // Verificar cache
  const fingerprint = computeCacheFingerprint(allPlayers, outputRoot);
  if (fs.existsSync(cachePath)) {
    try {
      const cached = JSON.parse(fs.readFileSync(cachePath, "utf-8"));
      if (cached._fingerprint === fingerprint) {
        const stats = cached.stats;
        console.log(`  ✓ Cross-stats: cache válido (${Object.keys(stats).length} jogadores)`);
        return stats;
      }
      console.log("  Cross-stats: cache invalidado, a regenerar...");
    } catch {
      console.log("  Cross-stats: cache corrompido, a regenerar...");
    }
  } else {
    console.log("  Cross-stats: a gerar pela primeira vez...");
  }
  ...
  for (const p of allPlayers) {
    ...
    if (done % step === 0 || done === total) {
      ...
      process.stdout.write(`\r  Cross-stats: ${done}/${total} (${pct}%) — ${elapsed}s`);
    }
  }
  process.stdout.write("\n");
  const elapsed = ((Date.now() - t0) / 1000).toFixed(1);
  console.log(`  ✓ Cross-stats completo: ${Object.keys(stats).length} jogadores em ${elapsed}s`);

  // Guardar cache
  try {
    require("./atomic-write").writeJsonAtomic(cachePath, { _fingerprint: fingerprint, stats }, 0);
    const sizeMB = (fs.statSync(cachePath).size / 1024 / 1024).toFixed(1);
    console.log(`  ✓ Cache guardado: cross-stats-cache.json (${sizeMB}MB)`);
  } catch (e) {
    console.error("  ⚠ Não consegui guardar cache:", e.message);
  }
