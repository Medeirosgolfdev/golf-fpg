...
function preparePlayerData(FED, allPlayers, crossStats) {
  ...
  // ── Normalizar formato novo (ResultsLST) → formato antigo (HCPWhsFederLST) ──
  // A FPG mudou a API em Fev 2026: campos têm nomes diferentes
  for (const r of rows) {
    // score_id / id
    ...
    // new_handicap ← calculated_exact_hcp (post-round HI, mesmo que exact_hcp no novo formato)
    if (r.new_handicap == null && r.calculated_exact_hcp != null) r.new_handicap = r.calculated_exact_hcp;
    // prev_handicap: no novo formato não existe separado; usar exact_hcp como aproximação
    ...
    if (!r.hcp_dateStr && r.score_dateStr) {
      // score_dateStr é "2026-01-18 08:48:00", hcp_dateStr era "2026-01-18"
      r.hcp_dateStr = r.score_dateStr.substring(0, 10);
    }
    // date ← score_date (para getPlayedAt)
    if (!r.date && r.score_date) r.date = r.score_date;
    // played_at ← score_date
    if (!r.played_at && r.score_date) r.played_at = r.score_date;
    // hcp_qualifying_round: manter como está (1=sim no novo formato)
    // calc_hcp_index ← calc_hcp_index (existe no novo mas frequentemente null)
    // Campos WHS que não existem no novo formato — ficam null
    // calc_low_hcp, calc_hcp_softcap, calc_hcp_hardcap, calc_score_avg,
    // calc_qty_scores, calc_qty_scores_calc, calc_adjust_total
  }
  ...
  for (let i = 0; i < qualifSorted.length; i++) {
    ...
    // Para a ronda mais recente: new_handicap não pode ser derivado de outra ronda
    // Usar calc_hcp_index se disponível, senão manter exact_hcp como aproximação
    if (r.new_handicap == null) {
      r.new_handicap = r.calc_hcp_index ?? r.exact_hcp;
    }
  }
  ...
  const newestQualifRow = sortedRows.find(r => r.hcp_qualifying_round === 1 || (r.hcp_qualifying_name || "").toLowerCase() === "sim") || {};
  const newestRow = sortedRows[0] || {};
  // Usar a ronda qualificativa mais recente para HCP info
  const hcpSource = newestQualifRow.new_handicap != null ? newestQualifRow : newestRow;
  const hcpInfo = {
    current: toNum(hcpSource.new_handicap) ?? toNum(newestRow.new_handicap) ?? null,
    lowHcp: toNum(hcpSource.calc_low_hcp) ?? toNum(newestRow.calc_low_hcp) ?? null,
    softCap: toNum(hcpSource.calc_hcp_softcap) ?? toNum(newestRow.calc_hcp_softcap) ?? null,
    hardCap: toNum(hcpSource.calc_hcp_hardcap) ?? toNum(newestRow.calc_hcp_hardcap) ?? null,
    scoreAvg: toNum(hcpSource.calc_score_avg) ?? toNum(newestRow.calc_score_avg) ?? null,
    qtyScores: toNum(hcpSource.calc_qty_scores) ?? toNum(newestRow.calc_qty_scores) ?? null,
    qtyCalc: toNum(hcpSource.calc_qty_scores_calc) ?? toNum(newestRow.calc_qty_scores_calc) ?? null,
    adjustTotal: toNum(hcpSource.calc_adjust_total) ?? toNum(newestRow.calc_adjust_total) ?? null
  };
