...
const whs = require("../scripts/lib/whs.cjs");
...
function preparePlayerData(FED, allPlayers, crossStats) {
  ...
  // A conta é a do site (src/utils/whsCalc.ts → indexFromWindow): melhores N
  // das últimas 20 (tabela 5.2a), extraordinários já aplicados e caps.
  ...
  if (qualifSorted.length > 0) {
    ...
    if (isSchema2) {
      // Janela das últimas 20 qualificativas, do mais recente para o mais
      // antigo, com o HI ANTES de cada volta (é contra ele que se mede o
      // resultado extraordinário que a FPG já aplicou).
      const last20Qualif = qualifSorted.slice(-20);
      const janela = [];
      for (const r of last20Qualif.slice().reverse()) {
        const sd = toNum(r.sgd) ?? toNum(r.score_differential);
        if (sd != null) janela.push({ sd, hi: toNum(r.prev_handicap) ?? toNum(r.exact_hcp) });
      }
      _derivedQtyScores = janela.length;

      // Low HI (Regra 5.8): o índice mais baixo dos 365 dias antes da última volta
      const lastDate = parseDotNetDate(lastQualif.score_date || lastQualif.hcp_date);
      const lows = qualifSorted
        .filter(r => {
          const d = parseDotNetDate(r.score_date || r.hcp_date);
          return lastDate && d && lastDate - d <= 365 * 24 * 3600 * 1000;
        })
        .map(r => toNum(r.exact_hcp))
        .filter(v => v != null);
      const idx = whs.indexFromWindow(janela, lows.length ? Math.min(...lows) : null);

      if (idx) {
        _derivedQtyCalc = idx.qtyCalc;
        _derivedScoreAvg = Math.round(idx.scoreAvg * 10) / 10;
        _derivedHcpFromSDs = idx.hi;
        _derivedLowHcp = Math.min(idx.hi, ...lows);
  ...
  if (melhPlayer) {
    ...
    if (Array.isArray(melhPlayer.treinos)) {
      ...
      for (let i = 0; i < treinos.length; i++) {
        ...
        const dateObj = dp.length === 3 ? new Date(Date.UTC(+dp[0], +dp[1]-1, +dp[2])) : null;
  ...
  for (const r of rounds) {
    ...
    // SD: estas voltas (treino/extra) não passaram pelo WHS, por isso não têm
    // SD oficial — ficam sem SD. Nunca se estima um (decisão 2026-09-15).
