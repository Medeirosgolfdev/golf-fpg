...
const { norm, parseDotNetDate, fmtDate, toNum, pickHcpFromRow, isMeaningful,
  ...
  normalizeWhsRows, normalizeCourse } = require("./helpers");
...
function preparePlayerData(FED, allPlayers, crossStats) {
  ...
  // Preferência: a que tem scoreOrigin preenchido (veio do WHS/FPG).
  // Treinos e extra_rounds excluídos.
  {
    ...
    for (let i = 0; i < rounds.length; i++) {
      ...
      if (!_seen.has(key)) {
        ...
        const rHasOrigin    = !!(r.scoreOrigin    && r.scoreOrigin    !== '');
        const prevHasOrigin = !!(prev.scoreOrigin && prev.scoreOrigin !== '');

        if (rHasOrigin && !prevHasOrigin) {
          _toRemove.add(prevIdx);
          _seen.set(key, i);
          console.log('  ↳ Dedup ' + r.date + ' ' + r.course + ' gross=' + g
            + ': mantém #' + r.scoreId + ' (origin="' + r.scoreOrigin + '")'
            + ', remove #' + prev.scoreId);
        } else {
          _toRemove.add(i);
          console.log('  ↳ Dedup ' + r.date + ' ' + r.course + ' gross=' + g
            + ': mantém #' + prev.scoreId + ' (origin="' + prev.scoreOrigin + '")'
            + ', remove #' + r.scoreId);
        }
      }
    }
