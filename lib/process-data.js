...
const { canonicalCourseName, rotateAroeira2RecordIfNeeded, resolveAroeiraIIByPar } = require("./course-aliases");
...
function preparePlayerData(FED, allPlayers, crossStats) {
  ...
  for (const [scoreId, rec] of cardByScoreId.entries()) {
    ...
    if (rec.course_description) {
      ...
      // "Aroeira II" é ambíguo — resolver pelo par[] do próprio rec.
      // Algumas rondas marcadas "Aroeira II" são na verdade No.1.
      const pars = [];
      for (let i = 1; i <= 18; i++) {
        const v = rec[`par_${i}`];
        if (v == null) break;
