...
const golfAllPath = path.join(process.cwd(), "scripts", "golf-all.js");
if (fs.existsSync(golfAllPath)) {
  info(`A correr: node scripts/golf-all.js --skip-download ${fed}`);
  try {
    execSync(`node scripts/golf-all.js --skip-download ${fed}`, { stdio: "inherit", cwd: process.cwd() });
  } catch (e) {
    warn(`Pipeline retornou erro (pode ser parcial)`);
  }
} else {
  warn("golf-all.js não encontrado — a saltar processamento");
  info("Corre manualmente: node scripts/golf-all.js --skip-download " + fed);
}
