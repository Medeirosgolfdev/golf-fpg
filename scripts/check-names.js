'use strict';
// check-names.js (v2: aceita multi-bucket via sufixos A/B/C/...)
// Cruza TODOS os verified-names*.json (final + checkpoints + buckets) com a
// cache uskids-member-history-*.json. Faz dedup automatico por mid (preferindo
// o resolvido pela API se houver multiplas entradas).
//
// Procura em data-archive + Downloads. Para cada "familia" (suffix), escolhe
// o ficheiro com mais results (defensivo contra checkpoints parciais).
//
// Uso:
//   node scripts/check-names.js
...
// Pattern de ficheiros aceites (final + checkpoint + bucket suffix):
//   verified-names.json
//   verified-names-checkpoint-NNN.json
//   verified-names-A.json
//   verified-names-A-checkpoint-NNNN.json
//   verified-names-B.json
//   verified-names-B-checkpoint-NNNN.json
//   ...
const FILE_RX = /^verified-names(?:-([A-Z]))?(?:-checkpoint-\d+)?\.json$/;

function familyKey(filename) {
  const m = filename.match(FILE_RX);
  if (!m) return null;
  return m[1] || '_';  // '_' = familia sem bucket (legacy/89 ALL CAPS)
}
...
// Agrupar por familia, escolher o de maior count por familia
...
const chosen = [];
console.log('> Candidatos verified-names: ' + cands.length + ' (' + families.size + ' familias)');
...
// Merge results de todas as familias escolhidas (dedup por mid; preferir resolved)
const allResults = new Map();  // mid -> result
for (const c of chosen) {
  const d = JSON.parse(fs.readFileSync(c.full, 'utf8'));
  for (const r of (d.results || [])) {
    const mid = String(r.mid);
    const existing = allResults.get(mid);
    // Se nao existir, ou existir mas nao resolved e este e' resolved, sobrescrever
    if (!existing || (!existing.resolved && r.resolved)) {
      allResults.set(mid, { ...r, _from: c.name });
    }
  }
}
console.log('> Merge de ' + chosen.length + ' ficheiros -> ' + allResults.size + ' mids unicos verificados');
...
// Imprimir apenas os "interessantes" (normalizados + mismatches + not_in_cache + api_unresolved)
console.log('> Resultados interessantes (excluindo match_exact silenciosos):');
for (const d of summary.details) {
  ...
  if (d.status === 'mismatch') {
    ...
    console.log('  ADD ' + d.mid + ' ' + d.api + ' (' + (d.country || '?') + ') - api so');
...
console.log('');
console.log('> Sumario consolidado (' + chosen.length + ' fontes):');
