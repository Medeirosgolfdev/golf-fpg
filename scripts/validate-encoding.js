...
function main() {
  ...
  /* 1. players.json (raiz) */
  const playersJsonPath = path.join(ROOT, "players.json");
  ...
  const publicPlayersPath = path.join(ROOT, "public", "data", "players.json");
  ...
  for (const rel of pipelineFiles) {
    const full = path.join(rel.startsWith("lib") ? ROOT : __dirname, rel);
