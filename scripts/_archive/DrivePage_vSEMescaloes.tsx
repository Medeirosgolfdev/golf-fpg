...
function ResumoTable(props: { tournaments: Tournament[]; playersDB: PlayersDB; sdLookup: SDLookup }) {
  ...
  const rows = useMemo(() => {
    ...
    return [...map.values()];
  }, [sorted, playersDB, sdLookup, challEscLookup]);
...
function AquaporView(props: { tournaments: Tournament[]; playersDB: PlayersDB; sdLookup: SDLookup }) {
  const { tournaments, playersDB, sdLookup } = props;
  if (!tournaments.length) return <div className="muted ta-center" style={{ padding: 24 }}>Sem torneios AQUAPOR.</div>;
  return (
    <RegionView tournaments={tournaments} playersDB={playersDB} sdLookup={sdLookup} label="Circuito AQUAPOR" />
  );
}
  ...
  return (
    <div className="tourn-layout">
      ...
      {series === "tour"
        ...
        : <AquaporView tournaments={aquaporT} playersDB={pdb} sdLookup={sdLookup} />}
