...
function buildBody(items) {
  ...
  const totT = items.reduce((a, i) => a + (i.digest.counts ? i.digest.counts.tournaments : 0), 0);
  const totP = items.reduce((a, i) => a + (i.digest.counts ? i.digest.counts.players : 0), 0);

  const out = [];
  // A menção garante o email mesmo com subscrição "Participating and @mentions".
  if (owner) out.push(`@${owner}`, "");
  out.push(
    `**${totT}** ${totT === 1 ? "torneio novo" : "torneios novos"} · **${totP}** ${totP === 1 ? "jogador nosso" : "jogadores nossos"} com scorecards novos.`,
    "",
    "---",
    "",
  );
...
async function main() {
  const items = loadPending().filter(
    (i) => i.digest && i.digest.counts && (i.digest.counts.tournaments || i.digest.counts.players),
  );
  if (!items.length) {
  ...
  const issue = await gh("POST", "/issues", { title, body, labels: ["resumo-dados"] });
...
module.exports = { buildBody, buildTitle };
