...
export function ScorecardLeaderboard({
  ...
  const siF9    = hasSI ? si!.slice(0, 9).reduce((a, b) => a + b, 0) : 0;
  const siB9    = hasSI && !is9 ? si!.slice(9, 18).reduce((a, b) => a + b, 0) : 0;
  const siTotal = hasSI ? si!.slice(0, nh).reduce((a, b) => a + b, 0) : 0;
